...
6. **Scope:** check `docs/PRD.md` non-goals before adding features. **Multi-host is now in scope** (see `docs/MULTIHOST.md`): manage remote hosts via an outbound `hivedock agent`, with local and remote sharing the identical `internal/hostops` core and security invariants. k8s, widgets, and auto-applying stack updates remain out.
...
8. **Byte-exact machine edits.** Any HiveDock-generated file write must be byte-identical to the original outside the intended scalar span, verified (reconstruct-and-compare + re-parse), or it aborts — never best-effort a file edit (docs/HARDENING.md §5.3). Editor saves are optimistic-locked (§5.1); machine edits diff-preview before applying (§5.2). Every file op is symlink-resolved and confined to STACKS_DIR (§4.2).
9. **No shell interpolation.** `exec.Command` argument arrays only — never `sh -c`/`bash -c` and never a shell container entrypoint. Enforced by a CI grep gate over non-test Go (docs/HARDENING.md §4.3). The self-update helper runs as the `hivedock apply-update` subcommand, not a shelled-out pipeline.
10. **Self-update applies only cosign-verified, digest-pinned images** strictly newer than the running version (docs/HARDENING.md §3). The check verifies the candidate's signature against the release-workflow identity before offering it (failure → alert, never a silent skip); the apply pulls the exact verified digest, never a tag. Don't reintroduce a tag-based `compose pull` self-update.
